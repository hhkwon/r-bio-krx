...
# Mar_CAP & Graph
page_2_m.c<- KRX_DATA_BIO_ATR_WPR_bycode[['셀트리온']][nrow(KRX_DATA_BIO_ATR_WPR_bycode[['셀트리온']]),][['cap_mil']]/10000000
format(page_2_m.c,big.mark=",",scientific=FALSE)
...
dygraph(split(page_2_m.c_xts, 'months')[[5]][1:5]) %>%
  dyLegend(show = "never") %>%
...
dygraph(split(page_2_t.v_xts, 'months')[[5]][1:5]) %>%
  dyLegend(show = "never") %>%
  dyAxis("x", drawGrid = FALSE, axisLineColor='#f2f2f2') %>%
  dyAxis("y", drawGrid = FALSE, axisLineColor='#f2f2f2') %>%
...
Row {data-height=500px, class='page_2_top_wrap'}
-------------------------------------
...
dygraph(split(page_2_m.c_xts, 'months')[[5]][1:5]) %>%
  ...
  dyAxis("x", drawGrid = FALSE, axisLineColor='#f2f2f2') %>%
  dyAxis("y", drawGrid = FALSE, axisLineColor='#f2f2f2') %>%
  dySeries('M.CAP', strokeWidth = 1, drawPoints = TRUE, pointSize = 2)
```

### Chart 2

```{r} 
renderText(c(page_2_m.c_last, page_2_m.c_max))
```
